...
async function bybit_upsert(guid, marketsSchemas) {
  logger.log(
    "info",
    `${guid} | ${new Date().toISOString()} | BYBIT Upsert started`
  );
  ...
  const response = await pool.query(query, (err, result) => {
    if (err) {
      ...
      logger.log(
        "info",
        `${guid} | ${new Date().toISOString()} | BYBIT Upsert ERROR: ${err}`
      );
    }
  });
  //client.release();
  logger.log(
    "info",
    `${guid} | ${new Date().toISOString()} | BYBIT Upsert ended`
  );
}

module.exports = bybit_upsert;
