...
async function chiliz_upsert(guid, marketsSchemas) {
  logger.log(
    "info",
    `${guid} | ${new Date().toISOString()} | CHILIZ Upsert started`
  );
  ...
  const response = await pool.query(query, (err, result) => {
    if (err) {
      ...
      logger.log(
        "info",
        `${guid} | ${new Date().toISOString()} | CHILIZ Upsert ERROR: ${err}`
      );
    }
  });
  //client.release();
  logger.log(
    "info",
    `${guid} | ${new Date().toISOString()} | CHILIZ Upsert ended`
  );
}

module.exports = chiliz_upsert;
