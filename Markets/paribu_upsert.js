...
async function paribu_upsert(guid, marketsSchemas) {
  logger.log(
    "info",
    `${guid} | ${new Date().toISOString()} | PARIBU Upsert started`
  );
  ...
  const response = await pool.query(query, (err, result) => {
    if (err) {
      ...
      logger.log(
        "info",
        `${guid} | ${new Date().toISOString()} | PARIBU Upsert ERROR: ${err}`
      );
    }
  });
  //client.release();
  logger.log(
    "info",
    `${guid} | ${new Date().toISOString()} | PARIBU Upsert ended`
  );
}

module.exports = paribu_upsert;
