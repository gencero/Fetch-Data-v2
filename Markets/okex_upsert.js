...
async function okex_upsert(guid, marketsSchemas) {
  logger.log(
    "info",
    `${guid} | ${new Date().toISOString()} | OKEX Upsert started`
  );
  ...
  const response = await pool.query(query, (err, result) => {
    if (err) {
      ...
      logger.log(
        "info",
        `${guid} | ${new Date().toISOString()} | OKEX Upsert ERROR: ${err}`
      );
    }
  });
  //client.release();
  logger.log(
    "info",
    `${guid} | ${new Date().toISOString()} | OKEX Upsert ended`
  );
}

module.exports = okex_upsert;
