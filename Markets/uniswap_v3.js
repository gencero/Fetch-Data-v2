...
async function getUniswap3Data(guid, url) {
  try {
    logger.log("info",`${guid} | ${new Date().toISOString()} | UNISWAP-V3 started`);
    console.log("url: " + url);
    ...
    return new Promise((resolve, reject) => {
      axios
            ...
            for (i in groupedData) {
                if (
                    ...
                    if (
                        ...
                        uniswap3Schema.parity = groupedData[i].symbol.toUpperCase();
                        uniswap3Schema.buy = toFixed(lowestAsk * result[0]["caprazbuy"]);
                        uniswap3Schema.sell = toFixed(highestBid * result[0]["caprazsell"]);
                        uniswap3Schema.hambuy = toFixed(lowestAsk);
                        uniswap3Schema.hamsell = toFixed(highestBid);
                        uniswap3Schema.caprazbuy = result[0]["caprazbuy"];
                        uniswap3Schema.caprazsell = result[0]["caprazsell"];
                        uniswap3Schema.base = "";
                        uniswap3Schema.contractaddress = groupedData[i].id;
        ...
        .catch((error) => {logger.log("info",`${guid} | ${new Date().toISOString()} | UNISWAP-V3 error | + ${error}`);
            reject("XXX UNISWAP-V3 ERR: " + error);
        });
    });
  } catch (error) {
    logger.log("info",`${guid} | ${new Date().toISOString()} | UNISWAP-V3 error | + ${error}`);
  }
}
