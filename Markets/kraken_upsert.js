...
async function kraken_upsert(guid, marketsSchemas) {
  logger.log(
    "info",
    `${guid} | ${new Date().toISOString()} | KRAKEN Upsert started`
  );
  ...
  const response = await pool.query(query, (err, result) => {
    if (err) {
      ...
      logger.log(
        "info",
        `${guid} | ${new Date().toISOString()} | KRAKEN Upsert ERROR: ${err}`
      );
    }
  });
  //client.release();
  logger.log(
    "info",
    `${guid} | ${new Date().toISOString()} | KRAKEN Upsert ended`
  );
}

module.exports = kraken_upsert;
