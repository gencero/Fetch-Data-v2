...
async function ftx_upsert(guid, marketsSchemas) {
  logger.log('info', `${guid} | ${new Date().toISOString()} | FTX Upsert started`);
  ...
  const response = await pool.query(query, (err, result) => {
    if (err){
      console.log(err);
      logger.log('info', `${guid} | ${new Date().toISOString()} | FTX Upsert ERROR: ${err}` );
    }
  });
  //client.release();  
  logger.log('info', `${guid} | ${new Date().toISOString()} | FTX Upsert ended`);
};

module.exports = ftx_upsert;
