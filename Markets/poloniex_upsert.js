...
async function poloniex_upsert(guid, marketsSchemas) {
//const poloniex_upsert = (guid, marketsSchemas) => {
  logger.log('info', `${guid} | ${new Date().toISOString()} | POLONIEX Upsert started`);
  ...
  const response = await client.query(query, (err, result) => {
    if (err){
      console.log(err);
      logger.log('info', `${guid} | ${new Date().toISOString()} | POLONIEX Upsert ERROR: ${err}` );
    }
  });
  client.release();  
  logger.log('info', `${guid} | ${new Date().toISOString()} | POLONIEX Upsert ended`);
  
};

module.exports = poloniex_upsert;
