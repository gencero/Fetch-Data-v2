...
async function getUniswap1_Block(){
    ...
    const response = await pool.query(query, async (err, result) => {
      ...
      .on("connected", ()=> logger.log('info', `${new Date().toISOString()} | UNISWAP-1 Connected to eth node, waiting for block`))
      .on("data", fetchData)
        ...
        const response = await pool.query(query, async (err, result) => {
          if (err){
            ...
            if (active == true) {
              ...
              logger.log('info', `${guid} | ${new Date().toISOString()} | UNISWAP-1 Block ${block.number} recieved, loading new data`);
              await fetch(guid, exchangeUrl);
            }else{
              logger.log('info', `${new Date().toISOString()} | UNISWAP-1 deactivated`);
            }
          }
        });
        //client.release();   
